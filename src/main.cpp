...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "INF01047 - 00316161 - Jordi Pujol Ricarte", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = matrices::Matrix_Translate(0.0f,-1.0f,0.0f)
              * matrices::Matrix_Scale(2.0f,1.0f,2.0f);
        glUniformMatrix4fv(model_uniform, 1 , GL_FALSE, glm::value_ptr(model));
...
void ComputeNormals(GameModel* model)
{
    ...
    for (size_t shape = 0; shape < model->shapes.size(); ++shape)
    {
        ...
        for (size_t triangle = 0; triangle < num_triangles; ++triangle)
        {
            ...
            // PREENCHA AQUI o cálculo da normal de um triângulo cujos vértices FEITO
